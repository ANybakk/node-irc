...
Client.prototype.connect = function ( retryCount, callback ) { // {{{
    ...
    if (typeof(callback) === 'function') {
      this.once('irc.registered', callback);
    }
    var self = this;
    self.chans = {};
    
    self.addListener("irc.raw", function (message) { // {{{
        switch ( message.command ) {
            case "001":
                ...
                self.emit('irc.registered', message);
                break;
            case "002":
            case "003":
            case "rpl_myinfo":
            ...
            case "PING":
                ...
                self.emit('irc.ping', message.args[0]);
                break;
            case "NOTICE":
                ...
                self.emit('irc.notice', from, to, text, message);
                ...
                modeList.forEach(function(mode) {
                    ...
                    if ( mode in self.prefixForMode ) {
                        ...
                        if ( adding ) {
                            ...
                            self.emit('irc.+mode', message.args[0], message.nick, mode, user, message);
                        }
                        else {
                        ...
                        if ( adding ) {
                            ...
                            self.emit('irc.+mode', message.args[0], message.nick, mode, modeArg, message);
                        }
                        else {
            ...
            case "NICK":
                ...
                self.emit('irc.nick', message.nick, message.args[0], channels, message);
                break;
            case "rpl_motdstart":
            ...
            case "rpl_endofnames":
                ...
                if ( channel ) {
                    self.emit('irc.names', message.args[1], channel.users);
                    self.emit('irc.names' + message.args[1], channel.users);
            ...
            case "330": // rpl_whoisaccount?
                ...
                break;
            case "rpl_endofwhois":
                self.emit('irc.whois', self._clearWhoisData(message.args[1]));
                break;
            case "rpl_liststart":
                self.channellist = [];
                self.emit('irc.channellist_start');
                break;
            case "rpl_list":
                var channel = {
                    name: message.args[1],
                ...
                self.emit('irc.channellist_item', channel);
                self.channellist.push(channel);
                break;
            case "rpl_listend":
                self.emit('irc.channellist', self.channellist);
                break;
            case "333":
                // TODO emit?
                var channel = self.chanData(message.args[1]);
                if ( channel ) {
                    ...
                    // channel, topic, nick
                    self.emit('irc.topic', message.args[1], channel.topic, channel.topicBy, message);
                }
                break;
            case "TOPIC":
                // channel, topic, nick
                self.emit('irc.topic', message.args[0], message.args[1], message.nick, message);
            ...
            case "JOIN":
                ...
                self.emit('irc.join', message.args[0], message.nick, message);
                self.emit('irc.join' + message.args[0], message.nick, message);
                if ( message.args[0] != message.args[0].toLowerCase() ) {
                    self.emit('irc.join' + message.args[0].toLowerCase(), message.nick, message);
                }
                break;
            case "PART":
                // channel, who, reason
                self.emit('irc.part', message.args[0], message.nick, message.args[1], message);
                self.emit('irc.part' + message.args[0], message.nick, message.args[1], message);
                if ( message.args[0] != message.args[0].toLowerCase() ) {
                    self.emit('irc.part' + message.args[0].toLowerCase(), message.nick, message.args[1], message);
                }
                if ( self.nick == message.nick ) {
                    var channel = self.chanData(message.args[0]);
            ...
            case "KICK":
                ...
                self.emit('irc.kick', message.args[0], message.args[1], message.nick, message.args[2], message);
                self.emit('irc.kick' + message.args[0], message.args[1], message.nick, message.args[2], message);
                if ( message.args[0] != message.args[0].toLowerCase() ) {
                    self.emit('irc.kick' + message.args[0].toLowerCase(), message.args[1], message.nick, message.args[2], message);
                }
            ...
            case "KILL":
                ...
                self.emit('irc.kill', nick, message.args[1], channels, message);
                break;
            case "PRIVMSG":
                var from = message.nick;
                var to   = message.args[0];
                ...
                self.emit('irc.message', from, to, text, message);
                if ( self.supported.channel.types.indexOf(to.charAt(0)) !== -1 ) {
                    self.emit('irc.message#', from, to, text, message);
                    self.emit('irc.message' + to, from, text, message);
                    if ( to != to.toLowerCase() ) {
                        self.emit('irc.message' + to.toLowerCase(), from, text, message);
                    }
                }
                if ( to == self.nick ) self.emit('irc.pm', from, text, message);
            ...
            case "INVITE":
                ...
                self.emit('irc.invite', channel, from, message);
                break;
            case "QUIT":
                if ( self.opt.debug )
                ...
                // who, reason, channels
                self.emit('irc.quit', message.nick, message.args[0], channels, message);
                break;
    ...
    self.addListener('irc.kick', function(channel, who, by, reason) {
        if ( self.opt.autoRejoin )
            self.send.apply(self, ['JOIN'].concat(channel.split(' ')));
    });
    self.addListener('irc.motd', function (motd) {
    ...
    self.conn.addListener("close", function() {
        ...
        if ( self.opt.retryCount !== null && retryCount >= self.opt.retryCount ) {
            ...
            self.emit( 'irc.abort', self.opt.retryCount );
            return;
        }
    ...
    self.conn.addListener("error", function(exception) {
        self.emit("irc.netError", exception);
    });
...
Client.prototype.join = function(channel, callback) { // {{{
    this.once('irc.join' + channel, function () {
...
Client.prototype.part = function(channel, callback) { // {{{
    if ( typeof(callback) == 'function' ) {
        this.once('irc.part' + channel, callback);
    }
    ...
    if (typeof text !== 'undefined') {
        text.toString().split(/\r?\n/).filter(function(line) {
            ...
            while ((messagePart = r.exec(line)) != null) {
                ...
                self.emit('irc.selfMessage', target, messagePart[0]);
            }
        });
    }
} // }}}
...
Client.prototype.whois = function(nick, callback) { // {{{
    if ( typeof callback === 'function' ) {
        ...
        this.addListener('irc.whois', callbackWrapper);
    }
...
Client.prototype._handleCTCP = function(from, to, text, type) {
    ...
    this.emit('irc.ctcp', from, to, text, type)
    this.emit('irc.ctcp-'+type, from, to, text)
    if (type === 'privmsg' && text === 'VERSION')
        this.emit('irc.ctcp-version', from, to)
    if (parts[0] === 'ACTION' && parts.length > 1)
        this.emit('irc.action', from, to, parts.slice(1).join(' '))
